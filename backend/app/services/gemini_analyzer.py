...
USER_PROMPT_TEMPLATE = """OCR распознал текст с этикетки:
<ocr_text>
{ocr_text}
</ocr_text>

Используй OCR-текст как основу для извлечения полей.
По изображению ОБЯЗАТЕЛЬНО дополни:
- визуальные элементы: знаки EAC, Мебиус, Бокал-вилка, штрихкод
- КБЖУ (пищевая/энергетическая ценность): ищи ВО ВСЁМ тексте И в изображении.
  Может быть в любом формате:
  · Строчный: "белки – 18, жиры – 8, углеводы – 0, калорийность – 140 ккал"
  · Таблица: столбцы Б / Ж / У / ккал с числами
  · Сокращения: "Б:18 Ж:8 У:0 Ккал:140" или "protein 18g fat 8g"
  · Числа могут идти через запятую, тире, точку, пробел
  Заполни protein/fat/carbs/kcal как числа (float). Если поле не указано — null.
  НЕ оставляй весь объект nutrition=null если хоть одно значение найдено.
